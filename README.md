...
目前實作 M1 的核心流程：私人任務看板、快速建立、編輯／刪除、跨欄拖曳與選單搬移、排序、欄位管理，以及 Supabase 持久保存與狀態歷史。這不代表完整 V1 或正式部署已完成。
...
1. 審查並以專案擁有者套用 `supabase/migrations/202609190001_board.sql`。正式環境須取得授權；migration 僅執行一次。
...
欄位底層保留 todo / doing / done 語意；刪除前須選擇同狀態替代欄位。任務預設 Regular，僅需標題。進出 Doing 與完成狀態均保存歷史；移回未完成時清除目前完成時間，原始轉換歷史仍保留。
