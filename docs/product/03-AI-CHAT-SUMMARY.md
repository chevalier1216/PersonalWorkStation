...
工作台的 AI 入口必須以瀏覽器開啟 ChatGPT 一般「對話」頁，嘗試預填工作台整理出的提示文字，並同步複製提示作為可靠 fallback。
...
工作台提供瀏覽器入口，開啟 `chatgpt.com` 一般對話頁，嘗試預填內容並將相同提示複製到剪貼簿。ChatGPT 未套用預填參數時，使用者直接貼上。
...
ChatGPT 會沿用帳號上次選擇的「對話／工作」模式；目前沒有可依賴的 URL 參數能由跨網域 Web App 強制切換。工作台必須提醒使用者確認「對話」，不得把 URL 參數當成模式已切換的證據。
