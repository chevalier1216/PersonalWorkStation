...
export function ExecutionCenter({
  ...
  const selectedRun =
    state.runs.find((item) => item.id === selectedRunId) ?? state.runs[0] ?? null;
  ...
  const projects = [...new Set(state.runs.map((item) => item.project).filter(Boolean))];
  ...
  return (
    <section className="execution-center" aria-label="AI 執行中心">
        ...
        <div className="execution-runs-layout">
          <div>
            <form className="run-create" onSubmit={create}>
              ...
              <label>
                Project
                <input value={project} maxLength={300} onChange={(e) => setProject(e.target.value)} />
              </label>
              <label>
                關聯 Task
              ...
              <p className="subtle">尚未連接 executor 時會顯示 Waiting External，不會假裝已執行。</p>
            </form>
            <div className="run-filters">
              <label>
                狀態
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as "all" | WorkflowStatus)}>
              ...
              <li key={item.id}>
                ...
                >
                  <span className={`workflow-status ${item.status}`}>
                    {statusSymbols[item.status]} {workflowStatusLabels[item.status]}
                  </span>
                  <strong>{item.title}</strong>
                  <code>{item.run_code}</code>
                  <small>{item.project || "未指定 Project"} · {runDuration(item)} 秒</small>
                </button>
              </li>
            ))}
          </ol>
      ...
      {view === "graph" && selectedRun && (
        <div className="workflow-graph" aria-label={`Graph ${selectedRun.run_code}`}>
          {nodes.map((node, index) => (
            <div className="graph-step" key={node.id}>
              ...
              {index < nodes.length - 1 && <span className="graph-edge">→</span>}
            </div>
          ))}
        </div>
      )}
      ...
      {view === "detail" && selectedRun && (
        <div className="workflow-detail">
          <section>
            ...
            <p><code>{selectedRun.run_code}</code> · {workflowStatusLabels[selectedRun.status]}</p>
            <p>Source：{selectedRun.source} · Executor：{selectedRun.executor || "未連接"}</p>
            {selectedRun.task_id && (
              <button onClick={() => openTask(selectedRun.task_id!)}>
                開啟關聯 Task：{tasks.find((task) => task.id === selectedRun.task_id)?.title ?? selectedRun.task_id}
              </button>
            )}
            {selectedRun.error && <p className="error" role="alert">{selectedRun.error}</p>}
          </section>
          {selectedNode && (
            <section>
              ...
              <dl>
                ...
                <dt>Status</dt><dd>{workflowStatusLabels[selectedNode.status]}</dd>
                <dt>Tool</dt><dd>{selectedNode.tool || "—"}</dd>
                <dt>Retry</dt><dd>{selectedNode.retry_count}</dd>
              </dl>
              <h4>Input</h4><pre>{jsonText(selectedNode.input)}</pre>
              <h4>Output</h4><pre>{jsonText(selectedNode.output)}</pre>
              <h4>Verification</h4><pre>{jsonText(selectedNode.verification)}</pre>
              {selectedNode.error && <><h4>Error</h4><pre className="error">{selectedNode.error}</pre></>}
              {selectedNode.status === "failed" && (
                <button disabled={busy} onClick={() => void run("retry_node", { node_id: selectedNode.id, reason: "使用者從執行中心重試" })}>
                  Retry
                </button>
              )}
            </section>
          ...
          <section>
            <h3>Logs</h3>
            {logs.length ? logs.map((log) => <p key={log.id}>{log.level} · {log.summary}</p>) : <p className="subtle">尚無 Log</p>}
            <h3>Artifacts</h3>
            {artifacts.length ? artifacts.map((artifact) => (
              <p key={artifact.id}>{artifact.kind} · {artifact.url ? <a href={artifact.url} target="_blank" rel="noreferrer">{artifact.label}</a> : artifact.label}</p>
          ...
          {gates.filter((gate) => gate.status === "open").map((gate) => (
            <form
              ...
              onSubmit={async (event) => {
                event.preventDefault();
                if (await run("resolve_human_gate", { gate_id: gate.id, response: gateResponse.trim() })) setGateResponse("");
              }}
            >
              <h3>Waiting Human</h3>
              <p>{gate.prompt}</p>
              <label>處理結果<textarea value={gateResponse} onChange={(e) => setGateResponse(e.target.value)} /></label>
              <button disabled={busy || !gateResponse.trim()}>記錄並繼續</button>
            </form>
          ))}
        </div>
      )}
...
export function ExecutionSummary({
  ...
  return (
    <section className="today-module" aria-label="AI 執行狀態">
      <div className="module-heading"><div><p className="eyebrow">AI EXECUTION</p><h2>AI 執行狀態</h2></div></div>
          ...
          const items = state.runs.filter(
            (run) =>
              ...
              (group.label !== "Completed Today" ||
                (run.finished_at && new Date(run.finished_at).toDateString() === today)),
          );
          return (
            <section key={group.label} aria-label={group.label}>
              ...
              {items.slice(0, 5).map((run) => (
                <button key={run.id} onClick={() => openRun(run.id)}>
                  <strong>{run.title}</strong><code>{run.run_code}</code>
                </button>
              ))}
            </section>
          );
